...
type internal DbImpl(config : IDbConfig) as this = 
    ...
    let checkCanDelete entityMeta = 
        if entityMeta.IdPropMetaList.IsEmpty then 
            raise <| FSharp.ORM.DbException(SR.SOMA4005(entityMeta.Type.FullName))
    ...
    member this.MakeTupleList (tupleMeta : TupleMeta) (reader : DbDataReader) = 
        let fieldCount = reader.FieldCount
        if fieldCount < tupleMeta.BasicElementMetaList.Length then raise <| FSharp.ORM.DbException(SR.SOMA4010())
        let columnIndexes = this.CreateColumnIndexes reader
    ...
    member this.MakeSingleList typ (reader : DbDataReader) = 
        let convertFromColumnToReturn (dbValue : obj) = 
            this.ConvertFromDbToClr dbValue typ null null (fun exn -> 
                ...
                raise <| FSharp.ORM.DbException(SR.SOMA4019(typ.FullName, typ.FullName), exn))
        seq { 
            while reader.Read() do
    ...
    member this.FindCore<'T, 'TResult> (idList : obj list) 
        ...
        if idList.IsEmpty then raise <| FSharp.ORM.DbException(SR.SOMA4004())
        let readerHandler, entityMeta = 
            let typ = typeof<'T>
            if Meta.isEntityType typ then 
                let entityMeta = Meta.makeEntityMeta typ dialect
                if entityMeta.IdPropMetaList.IsEmpty then raise <| FSharp.ORM.DbException(SR.SOMA4005(typ.FullName))
                else 
                    if entityMeta.IdPropMetaList.Length <> idList.Length then 
                        raise <| FSharp.ORM.DbException(SR.SOMA4003(entityMeta.IdPropMetaList.Length, idList.Length))
                this.MakeEntityList entityMeta, entityMeta
            else raise <| FSharp.ORM.DbException(SR.SOMA4002())
        ...
        if enumerator.MoveNext() then 
            ...
            if enumerator.MoveNext() then raise <| FSharp.ORM.DbException(SR.SOMA4016(ps.Text, ps.Parameters))
    ...
    member this.FailCauseOfTooManyAffectedRows ps rows = 
        raise <| FSharp.ORM.DbException(SR.SOMA4012(rows, ps.Text, ps.Parameters))
    
    member this.GetEntityMeta typ = 
        if not <| Meta.isEntityType typ then raise <| FSharp.ORM.DbException(SR.SOMA4007())
    ...
    member this.Call<'T when 'T : not struct>(procedure : 'T) = 
        ...
        let procedureMeta = 
            if not <| Meta.isProcedureType typ then raise <| FSharp.ORM.DbException(SR.SOMA4021())
