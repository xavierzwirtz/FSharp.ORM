...
module FindTest = 
  ...
  [<Test>]
  let ``find``() =
    let department = Oracle.find<Department> [2]
  ...
  let ``find : id type is option``() =
    let employee = Oracle.find<Employee> [1]
    printfn "%A" employee

  [<Test>]
  let ``find : not found``() =
    try
      Oracle.find<Department> [99] |> ignore
      fail ()
    with 
  ...
  let ``findWithVersion : not found``() =
    try
      Oracle.findWithVersion<Department> [99] 0 |> ignore
      fail ()
    with
  ...
  let ``findWithVersion : optimistic lock confliction``() =
    try
      Oracle.findWithVersion<Department> [2] 99 |> ignore
      fail ()
    with
  ...
  let ``tryFindWithVersion``() =
    let department = Oracle.tryFindWithVersion<Department> [2] 0
    printfn "%A" department
    assert_equal (Some { DepartmentId = 2; DepartmentName = "Sales"; VersionNo = 0; }) department

  [<Test>]
  let ``tryFindWithVersion : by composite id``() =
    let employee = Oracle.tryFindWithVersion<CompKeyEmployee> [2; 12] 0
    printfn "%A" employee
    assert_equal (Some { EmployeeId1 = 2; EmployeeId2 = 12; EmployeeName = "Smith"; VersionNo = 0; }) employee

  [<Test>]
  let ``tryFindWithVersion : not found``() =
    let department = Oracle.tryFindWithVersion<Department> [99] 0
    assert_true department.IsNone

  [<Test>]
  let ``tryFindWithVersion : optimistic lock confliction``() =
    try
      Oracle.tryFindWithVersion<Department> [2] 99 |> ignore
      fail ()
    with
    | OptimisticLockException  _ as ex -> printfn "%A" ex
