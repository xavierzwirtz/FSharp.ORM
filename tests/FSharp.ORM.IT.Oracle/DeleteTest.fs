...
module DeleteTest = 
  ...
  let ``delete : no id``() =
    ...
    try
      Oracle.delete { NoId.Name = "aaa"; VersionNo = 0 } |> ignore
      fail ()
    with 
  ...
  let ``delete : no version``() =
    use ts = new TransactionScope()
    Oracle.delete { NoVersion.Id = 1; Name = "aaa" }

  [<Test>]
  let ``delete : incremented version``() =
    use ts = new TransactionScope()
    let department = Oracle.find<Department> [1]
    Oracle.delete department
  ...
  let ``delete : incremented version : optimistic lock confliction``() =
    ...
    try
      Oracle.delete department |> ignore
      fail ()
    with 
  ...
  let ``deleteIgnoreVersion``() =
    ...
    Oracle.deleteWithOpt department (DeleteOpt(IgnoreVersion = true))
  ...
  let ``deleteIgnoreVersion : no affected row``() =
    ...
    try
      Oracle.deleteWithOpt department (DeleteOpt(IgnoreVersion = true)) |> ignore
      fail ()
    with 
