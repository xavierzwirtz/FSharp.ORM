...
module QueryTest = 
  ...
  let ``select with ADO.NET fashion``() =
    let factory = MsSqlCe.config.DbProviderFactory
    use connection = factory.CreateConnection()
    connection.ConnectionString <- MsSqlCe.config.ConnectionString
    connection.Open()
  ...
  let ``query : 1 tuple``() =
    use con = MsSqlCe.createConnection()
    let departments = 
      MsSqlCe.query<int * string * int> con "
  ...
  let ``query : 1 tuple : record mixed``() =
    use con = MsSqlCe.createConnection()
    let departments = 
      MsSqlCe.query<string * Employee> con "
  ...
  let ``queryOnDemand``() =
    use con = MsSqlCe.createConnection()
    let departments = 
      MsSqlCe.queryOnDemand<Department> con "
        select * from Department order by DepartmentId
        " [] 
    departments |> Seq.iter (printfn "%A")
