...
module DeleteTest = 
  ...
  let ``delete : no id``() =
    ...
    try
      MySql.delete { NoId.Name = "aaa"; VersionNo = 0 } |> ignore
      fail ()
    with 
  ...
  let ``delete : no version``() =
    use ts = new TransactionScope()
    MySql.delete { NoVersion.Id = 1; Name = "aaa" }

  [<Test>]
  let ``delete : incremented version``() =
    use ts = new TransactionScope()
    let department = MySql.find<Department> [1]
  ...
  let ``delete : incremented version : optimistic lock confliction``() =
    ...
    try
      MySql.delete department |> ignore
      fail ()
    with 
  ...
  let ``deleteIgnoreVersion``() =
    ...
    MySql.deleteWithOpt department (DeleteOpt(IgnoreVersion = true))
  ...
  let ``deleteIgnoreVersion : no affected row``() =
    ...
    try
      MySql.deleteWithOpt department (DeleteOpt(IgnoreVersion = true)) |> ignore
      fail ()
    with 
